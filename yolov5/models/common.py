...
class h_swish(nn.Module):  # new
    def __init__(self, inplace=True):
        super().__init__()
        self.act = h_sigmoid(inplace)

    def forward(self, x):
        return x * self.act(x)


class CoordinateAttention(nn.Module):  # changed: rewritten to avoid AdaptiveAvgPool2d
    def __init__(self, inp, oup, reduction=32):
        super().__init__()
        mip = max(8, inp // reduction)

        # NOTE: removed self.pool_h and self.pool_w (used AdaptiveAvgPool2d)  # changed from: using AdaptiveAvgPool2d
    ...
    def forward(self, x):
        ...
        # --- changed from using AdaptiveAvgPool2d to using .mean for deterministic-friendly behavior ---
        # old:
        # x_h = self.pool_h(x)                  # (n, c, h, 1)
        # x_w = self.pool_w(x).transpose(2, 3)  # (n, c, 1, w)
        #
        # new:
        x_h = x.mean(dim=3, keepdim=True)                  # (n, c, h, 1)  # changed from: pool_h
        x_w = x.mean(dim=2, keepdim=True).transpose(2, 3)  # (n, c, 1, w)  # changed from: pool_w
...
class C3CA(C3):  # new, C3 with Coordinate Attention
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, reduction=32):
        # this line is same as C3 init
        super().__init__(c1, c2, n=n, shortcut=shortcut, g=g, e=e)
        # new CA module
        self.ca = CoordinateAttention(c2, c2, reduction=reduction)

    def forward(self, x):
        y = super().forward(x)  # C3 forward (pretrained part)
        return self.ca(y)       # apply Coordinate Attention
# =========================
# new: Efficient Multi-Scale Attention (EMA) module
# =========================
class EMA(nn.Module):
    def __init__(self, channels, c2=None, factor=32):
        """
        Efficient Multi-Scale Attention (EMA) without AdaptiveAvgPool2d
        to avoid non-deterministic CUDA backward.

        channels: input/output channels (C)
        c2: kept for API symmetry, not used
        factor: number of groups used for grouped processing
        """
        super().__init__()
        self.groups = factor  # number of groups
        ...
        # changed from: using AdaptiveAvgPool2d (non-deterministic backward)
        # self.agp = nn.AdaptiveAvgPool2d((1, 1))
        # self.pool_h = nn.AdaptiveAvgPool2d((None, 1))
        # self.pool_w = nn.AdaptiveAvgPool2d((1, None))

        # grouping + multi-scale convs
        # GroupNorm(num_groups, num_channels); here both = group_channels
    ...
    def forward(self, x):
        """
        x: (B, C, H, W), returns attention-refined feature of same shape.
        """
        b, c, h, w = x.shape
        g = self.groups
        assert c % g == 0
        ...
        # group the channels: (B, C, H, W) -> (B*g, Cg, H, W)
        x_grouped = x.view(b * g, c_g, h, w)

        # ----------------------------
        # Directional pooling (deterministic)
        # ----------------------------
        # changed from: x_h = self.pool_h(x_grouped)
        #               x_w = self.pool_w(x_grouped)
        # to using mean over width/height respectively
        x_h = x_grouped.mean(dim=3, keepdim=True)        # avg over W -> (B*g, Cg, H, 1)
        x_w = x_grouped.mean(dim=2, keepdim=True)        # avg over H -> (B*g, Cg, 1, W)
        x_w = x_w.transpose(2, 3)                        # (B*g, Cg, W, 1)

        # fuse H and W descriptors: concat along spatial dim -> (B*g, Cg, H+W, 1)
        y = torch.cat([x_h, x_w], dim=2)

        # normalize + two conv branches (1x1 and 3x3)
        y = self.gn(y)
        y_1 = self.conv1x1(y)
        y_3 = self.conv3x3(y)

        # combine branches
        y = y_1 + y_3   # (B*g, Cg, H+W, 1)

        # split back to height and width attention
        y_h, y_w = torch.split(y, [h, w], dim=2)  # (B*g, Cg, H,1), (B*g, Cg, W,1)
        y_w = y_w.transpose(2, 3)                # (B*g, Cg, 1,W)

        # compute attention maps and apply to input
        a_h = self.softmax(y_h.view(b * g, c_g, h))      # along H
        a_w = self.softmax(y_w.view(b * g, c_g, w))      # along W

        # reshape back to spatial maps
        a_h = a_h.view(b * g, c_g, h, 1)
        a_w = a_w.view(b * g, c_g, 1, w)

        # apply attention to grouped features
        out = x_grouped * a_h * a_w  # element-wise
...
# =========================
# new: C3 with EMA attention (wrapper)
# =========================
class C3EMA(C3):  # new, C3 with EMA attention
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, factor=32):
        # this line is same as C3 init  # changed from: plain C3 without EMA
        super().__init__(c1, c2, n=n, shortcut=shortcut, g=g, e=e)
        self.ema = EMA(c2, factor=factor)  # new EMA block

    def forward(self, x):
        y = super().forward(x)  # C3 forward (pretrained part)
        return self.ema(y)      # apply EMA attention
